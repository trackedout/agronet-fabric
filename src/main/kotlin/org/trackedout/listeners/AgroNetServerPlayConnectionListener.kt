...
class AgroNetServerPlayConnectionListener(private val fetchPlayerDeckCommand: FetchPlayerDeckCommand) : ServerPlayConnectionEvents.Join {
    ...
    override fun onPlayReady(handler: ServerPlayNetworkHandler?, sender: PacketSender?, server: MinecraftServer?) {
        ...
        if (handler?.player != null) {
            fetchPlayerDeckCommand.execute(handler.player!!)
        }
    }
}